...
func New(_ context.Context, cfg *config.Config, logger *slog.Logger, opts Options) (*App, error) {
	...
	repo, k8sClient, err := a.buildStore()
	if err != nil {
...
func (a *App) buildStore() (store.NetworkRepository, kubernetes.Interface, error) {
	...
	return k8s.NewK8sNetworkStore(k8sClient, k8sCfg, a.logger), k8sClient, nil
}
