...
func NewClientCredentialsTokenSource(tokenEndpoint, clientID, clientSecret string, httpClient *http.Client) *ClientCredentialsTokenSource {
	if httpClient == nil {
		httpClient = &http.Client{
			...
			// Refuse all redirects: fetchToken's POST body carries
			// clientSecret, and Go's default client resends the same
			// method+body to a 307/308 Location, including cross-origin. A
			// caller-supplied httpClient is responsible for its own
			// redirect policy (REQ-DCM-261).
...
func (c *ClientCredentialsTokenSource) fetchToken(ctx context.Context) (string, time.Time, error) {
	...
	if resp.StatusCode != http.StatusOK {
		// Read a bounded amount of the body (also used to surface any OAuth2
		// error fields) before the deferred close, for keep-alive reuse.
		body, _ := io.ReadAll(io.LimitReader(resp.Body, maxDrainBytes))
		...
		if err := json.Unmarshal(body, &errBody); err == nil && errBody.Error != "" {
			if errBody.ErrorDescription != "" {
				return "", time.Time{}, fmt.Errorf("token endpoint returned HTTP %d: %s: %s", resp.StatusCode, errBody.Error, errBody.ErrorDescription)
			}
			return "", time.Time{}, fmt.Errorf("token endpoint returned HTTP %d: %s", resp.StatusCode, errBody.Error)
		}
...
// close, for keep-alive reuse and to cap the captured OAuth2 error body.
