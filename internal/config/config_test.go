...
var _ = Describe("DCM outbound authentication", Label("unit"), func() {
	...
	Describe("UsesInsecureAuthEndpoint", func() {
		It("returns true for an http:// endpoint with full client-credentials config (TC-CFG-UT-AUTH-090)", func() {
			setValidEnv()
			GinkgoT().Setenv("DCM_AUTH_TOKEN_ENDPOINT", "http://keycloak:8080/token")
			GinkgoT().Setenv("DCM_AUTH_CLIENT_ID", "agent-client")
			GinkgoT().Setenv("DCM_AUTH_CLIENT_SECRET", "super-secret")
			...
			cfg, err := config.Load()
			Expect(err).NotTo(HaveOccurred())
			Expect(cfg.DCM.UsesInsecureAuthEndpoint()).To(BeTrue())
		})

		It("returns false for an https:// endpoint with full client-credentials config (TC-CFG-UT-AUTH-090)", func() {
			setValidEnv()
			GinkgoT().Setenv("DCM_AUTH_TOKEN_ENDPOINT", "https://keycloak:8443/token")
			GinkgoT().Setenv("DCM_AUTH_CLIENT_ID", "agent-client")
			GinkgoT().Setenv("DCM_AUTH_CLIENT_SECRET", "super-secret")

			cfg, err := config.Load()
			Expect(err).NotTo(HaveOccurred())
			Expect(cfg.DCM.UsesInsecureAuthEndpoint()).To(BeFalse())
		})
	})
})
