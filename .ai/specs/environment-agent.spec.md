...
The `retry.Processor` handles retry-subject reprocessing only (`ProcessOnTransition`
and `ProcessOnRestart`). Main-subject consumption, cancel-subject consumption,
`MaxDeliver`-exceeded termination, and main-path handler timeouts are owned by
`messaging.Client` (see REQ-MSG-115, REQ-RCM-160, REQ-RCM-180).
